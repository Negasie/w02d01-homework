...
//	5.	Create a new array called bondTitles with only the titles 
//	of the Bond films, and console.log the new array.
	
	const bondTitles = []
	for(let key in bondFilms) {
		bondTitles[key] = bondFilms[key].title;	
	console.log(bondTitles[key]);
}



//	6.	Create a new array oddBonds, of only the Bond films released 
//	on odd-numbered years.
	const oddBonds = []
		for (let key in bondFilms) {
			oddBonds[key] = bondFilms[key].year;
			if(oddBonds[key] % 2 != 0){
				console.log(oddBonds[key]);
			}
			
		}


//	7.	Determine the total cumulative gross of the Bond franchise, 
//		and console.log the result.

//		Hint: To make the grosses into usable numbers, look into the 
...
for (let i = 0; i< bondFilms.length; i++){
	  ...
	  bondFilms[i].gross = bondFilms[i].gross.replace(",", "");
  	  ...
  	  totalGrossSum = totalGrossSum + parseInt(bondFilms[i].gross);
